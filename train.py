...
nn.add_layer(Layer(n_input=X_train.shape[1], n_neurons=90, activation='sigmoid', weights_seed=w_seed, bias_seed=b_seed))
nn.add_layer(Layer(n_input=90, n_neurons=90, activation='sigmoid', weights_seed=w_seed, bias_seed=b_seed))
nn.add_layer(Layer(n_input=90, n_neurons=y_train.shape[1], activation='sigmoid', weights_seed=w_seed, bias_seed=b_seed))
nn.add_layer(Layer(n_input=y_train.shape[1], n_neurons=y_train.shape[1], activation='softmax', weights_seed=w_seed, bias_seed=b_seed))
